...
//        如果前端验证码不匹配
        ...
        if(vCode!=null){
            ...
            if (status == null) {//该邮箱不存在，即允许注册
                ...
                //System.out.println("注册的时候的验证码"+code);
                //return "1";
            } else {
    ...
    public String register(Customer customer, HttpSession httpSession,Model model) {
        ...
        //验证失败，再次跳转到注册界面
        model.addAttribute("RegisterEmailCodeStatus","邮箱验证不正确!");
        return "register";
    }
    ...
    public String sendReturnPswdEmail(String email,HttpSession httpSession,String validatedCode){
        ...
        if(vCode!=null){
            ...
            if (status == null) {//该邮箱不存在，不允许发送邮箱验证码
                ...
                //System.out.println("重置密码邮箱验证码:"+code);
